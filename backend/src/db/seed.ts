...
async function main() {
  ...
  await prisma.order.createMany({
    data: [
      ...
      { restaurantId: restaurant.id, driverId: d1.id, customerAddress: '18 Qasr El Aini St',
        totalCashToCollect: 9500, status: 'Delivered', rating: 4,
        assignedAt: new Date(Date.now() - 33 * 60000), deliveredAt: new Date() },
